...
export default function ArtistDetail(props: coverProps) {
  ...
  const defaultData: serverSideProps = {
    artistId: Number(temp),
    intro: "",
    subtitle: "",
    content: "",
    profile: "",
  ...
  const [usingData, setData] = useState(defaultData);
  const { data } = props;
  useEffect(() => {
    setData(data);
  });
  ...
  return (
    <>
      <Seo title={temp} />
      <ArtistDetailPage
        artistId={usingData.artistId}
        intro={usingData.intro}
        subtitle={usingData.subtitle}
        content={usingData.content}
        profile={usingData.profile}
        background={usingData.background}
        youtube={usingData.youtube}
        detail={usingData.detail}
        artworks={usingData.artworks}
      />
    </>
  );
}
